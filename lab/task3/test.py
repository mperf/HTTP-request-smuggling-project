...
class test(unittest.TestCase):
    def test1(self):
        try:    
            ...
            self.assertTrue(b"message has been posted" in response2)
            self.assertTrue(b"YWRtaW46YWRtaW4xMjM=" in response2)
        except Exception as e:
            print(f"Error: {e}")
            print("TASK 3.2 ATTACK RESPONSE:")
            print('=' * 60)
            print(response1.decode('utf-8', errors='ignore')[:300])
            print()
            print("TASK 3.2 VICTIM RESPONSE:")
            print('=' * 60)
